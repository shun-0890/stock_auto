...
Write-Step "プルリクエスト作成"

$prExists = gh pr list --head $Branch --base main --json number --jq '.[0].number' 2>$null
if ($prExists) {
    Write-Skip "PR は既に作成済みです（#${prExists}）"
} else {
    $prBody = @"
## デイリーリサーチ：$Date
...
### 生成ファイル
- macro.md（マクロ分析）
- screening.md（スクリーニング）
- evaluation.md（銘柄評価）
- deepdive_*.md（詳細調査）
- article_macro.md / article_screening.md（記事）
"@
    gh pr create --title "デイリーリサーチ $Date" --body $prBody --base main --head $Branch
    if ($?) { Write-Success "PR 作成完了 ($Branch → main)" } else { Write-Fail "PR 作成失敗" }
}
